...
public partial class TotalTimelossSettings : UserControl
{
    ...
    // When true: shows the labels row (Sob / Tl / Bpt) above the values.
    // When false: shows only the values row (single-row mode).
    ...
    public TotalTimelossSettings()
    {
        ...
        // Defaults
        TextColor         = Color.FromArgb(255, 255, 255);   // white labels
        OverrideTextColor = false;
        TimeColor         = Color.FromArgb(255, 255, 255);   // white values
        OverrideTimeColor = false;
        Accuracy          = TimeAccuracy.Hundredths;          // show .XX decimals
        BackgroundColor   = Color.Transparent;
        BackgroundColor2  = Color.Transparent;
        BackgroundGradient = GradientType.Plain;
        Display2Rows      = true;                             // show labels by default
    ...
    private void rdoHundredths_CheckedChanged(object sender, EventArgs e)  => UpdateAccuracy();
    private void rdoTenths_CheckedChanged(object sender, EventArgs e)       => UpdateAccuracy();
    private void rdoSeconds_CheckedChanged(object sender, EventArgs e)      => UpdateAccuracy();
    private void rdoMilliseconds_CheckedChanged(object sender, EventArgs e) => UpdateAccuracy();

    private void UpdateAccuracy()
    {
        if (rdoSeconds.Checked)           Accuracy = TimeAccuracy.Seconds;
        else if (rdoTenths.Checked)       Accuracy = TimeAccuracy.Tenths;
        else if (rdoHundredths.Checked)   Accuracy = TimeAccuracy.Hundredths;
        else                              Accuracy = TimeAccuracy.Milliseconds;
    }
    ...
    public void SetSettings(XmlNode node)
    {
        ...
        Display2Rows      = SettingsHelper.ParseBool(element["Display2Rows"], true);
    }
